...
- The pure-PIL format/size checks ALWAYS work (only Pillow required).
- numpy + OpenCV (cv2) + rembg are imported LAZILY inside the methods that
  need them. If any of those optional deps are missing, the corresponding
  stage (quality / background-removal / object-detection) is skipped with a
  logged warning instead of crashing the import or the whole pipeline.
"""
...
class ImageValidator:
    ...
    Optional heavy deps (numpy, cv2, rembg) are imported lazily so the
    validator still loads (and the format/size checks still run) on a host
    ...
    def validate_and_process(
        ...
        try:
            ...
            # Step 2: Basic format validation (pure PIL - always runs)
            format_check = self._validate_format(image)
            if not format_check["valid"]:
                return format_check

            # Step 3: Image quality checks (needs numpy + cv2)
            quality_check = self._validate_quality(image)
            if not quality_check["valid"]:
                return quality_check

            # Step 4: Remove background (needs rembg). If unavailable, fall
            # back to the original image and skip object detection/cropping.
            no_bg_image = self._remove_background(image)
            if no_bg_image is None:
                logger.warning(
                    "Background removal unavailable - skipping object "
                    "detection and cropping; accepting image as-is."
                )
                validation_metadata = {
            ...
            # Step 5: Detect objects (needs numpy + cv2)
            object_detection = self._detect_objects(no_bg_image)
            if not object_detection["valid"]:
                return object_detection

            # If object detection was skipped (deps missing), accept the
            # background-removed image without cropping.
            if not object_detection.get("detected", True):
                validation_metadata = {
                    "original_size": original_size,
                    "processed_size": no_bg_image.size,
                    "num_objects_detected": 0,
                    "object_area_pixels": 0,
                    "blur_score": quality_check.get("blur_score"),
                    "validation_passed": True,
    ...
    def _validate_format(self, image: Image.Image) -> Dict:
        ...
        Pure PIL - no optional dependencies required.
        """
        # Check image mode
    ...
    def _validate_quality(self, image: Image.Image) -> Dict:
        ...
        Requires numpy + cv2. If either is missing, the quality stage is
        skipped (treated as a pass) with a logged warning.
        """
        try:
            import numpy as np
            import cv2
            ...
            logger.warning(
                f"Quality checks skipped (missing dependency: {e}). "
                "Install numpy + opencv-python-headless to enable them."
            )
            return {"valid": True, "blur_score": None}
    ...
    def _remove_background(self, image: Image.Image):
        ...
        Returns RGBA image with transparent background, or ``None`` if rembg
        (or its onnxruntime backend) is unavailable so the caller can degrade
        gracefully.
        """
        try:
            ...
            logger.warning(
                f"Background removal skipped (missing dependency: {e}). "
                "Install rembg + onnxruntime to enable it."
            )
            return None
    ...
    def _detect_objects(self, image: Image.Image) -> Dict:
        ...
        Requires numpy + cv2. If either is missing, detection is skipped and
        a ``detected: False`` flag is returned so the caller can accept the
        ...
        try:
            ...
            logger.warning(
                f"Object detection skipped (missing dependency: {e}). "
                "Install numpy + opencv-python-headless to enable it."
            )
            return {"valid": True, "detected": False}
    ...
    def _crop_and_center(
        ...
        Requires numpy + cv2 (only reached when object detection ran, which
        already guarantees both are importable).
        """
        import numpy as np
        import cv2
